...
## Current State
**Status**: Migrating to Supabase-only architecture

The application is being refactored to:
- Use **Supabase Auth** with Google OAuth (replacing Replit Auth)
- Use **Supabase PostgreSQL** with Row Level Security (replacing direct PostgreSQL)
- Use **Supabase Edge Functions** for AI generation and Stripe (keeping API keys secure)
- Enable **pure frontend deployment** (Vercel, Netlify, Replit static)
...
- **AI Generation**: Supabase Edge Function calling Claude API
- **Payments**: Supabase Edge Function calling Stripe API (optional)
- **File Processing**: Supabase Edge Function for PDF/DOCX text extraction
...
- **Pages**:
  ...
  - `/subscribe` - Stripe subscription checkout (optional)
